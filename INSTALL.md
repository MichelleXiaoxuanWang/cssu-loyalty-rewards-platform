...
======================= 2025/03/31 ============================
Quercus:
INSTALL: This document should include a detailed description of any preparation needed for your code to run in a new environment. Include:
- Prerequisite packages to install.
- How to set up the backend and frontend.
- Information on how to deploy your website.
  - The backend server can run on the same (virtual) machine as the web server for the frontend, and is preferred for simplicity.
...
# Prerequisite packages to install
## Backend
All dependencies are listed in the `backend/package.json` file:
```
"dependencies": {
    "@prisma/client": "^6.4.1",
    "bcrypt": "^5.1.1",
...
1. navigate to the `backend` directory. 
    ...
    cd backend
    ```
2. [Optional but recommended] create a `.env` file in the `backend` directory, as shown in the `backend/.env.example` file. You can specify the port and client URL as you wish. For example,
    ```
    PORT=8000
...
1. navigate to the `frontend` directory. 
    ...
    cd frontend
    ```
2. [Optional but recommended] create a `.env` file in the `frontend` directory, as shown in the `frontend/.env.example` file. You should specify the API URL as the URL of the backend server. For example (if you are running the backend server on the same machine using port 8000):
    ```
...
3. install the dependencies, and start the server:
    ...
    npm run dev
    ```
4. You can now access the frontend server at the url specified in the terminal. By default, it should be `http://localhost:5173`.
